    ...
    return {
        readDirectory: async uri => {
            ...
            return commit.tree.entries.map(
                ({ name, isDirectory }) =>
                    // See comment on MarkupKind.PlainText in extensionHost.ts ("The const enum
                    // MarkupKind...") for why using these constants is necessary.
                    [name, isDirectory ? 2 : 1] as [string, FileType]
            )
        },
        readFile: async uri => {
            const { repo, rev, path } = resolveURI(uri.toString())
