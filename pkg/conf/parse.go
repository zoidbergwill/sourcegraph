...
func ParseConfig(data conftypes.RawUnifiedConfiguration) (*UnifiedConfiguration, error) {
	cfg := &UnifiedConfiguration{
		Deployment: data.Deployment,
	}
	if err := parseConfigData(data.Core, &cfg.Core); err != nil {
		return nil, err
	}
	if err := parseConfigData(data.Core, &cfg.SiteConfiguration); err != nil {
		return nil, err
	}
	return cfg, nil
}
