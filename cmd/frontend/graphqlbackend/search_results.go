				...
				for _, r := range fileResults {
					...
					if ok {
						...
						for _, lm := range r.LineMatches() {
							...
							highlightedBody := r.GetHighlightedWithContext(ctx, lm.JLineNumber)
							matches = append(matches, &GenericSearchMatchResolver{url: url, body: lm.Preview(), path: &r.JPath, highlights: highlights, highlightedBody: &highlightedBody})
						}
						r.results = matches
						r.icon = fileIcon
						r.label = label
