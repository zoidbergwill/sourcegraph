...
interface GenericSearchResult {
    ...
    url: String!
    # Additional details that are rendered less prominently.
    detail: String
...
type GenericSearchMatch {
    # URL for the individual result match. If there is at most one match per result, this should be the same as the result's url.
    url: String!
    # Raw string of the result.
    body: String!
    # {Line, character, length} objects used to specify the offsets of each match in the body. Used to highlight matches in lines.
    # Can be left empty.
...
# A file match.
type FileMatch implements GenericSearchResult {
    # Base64 data uri to an icon.
    icon: String!
    # A markdown string that is rendered prominently.
    label: String!
